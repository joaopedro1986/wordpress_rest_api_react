...
class SimpleJWTLoginSettings {
	...
	private function initAuthenticationFromPost() {
		$this->assignSettingsPropertyFromPost( null,'allow_authentication', null,'allow_authentication',
		...
		$this->assignSettingsPropertyFromPost( null,'jwt_auth_refresh_ttl', null,'jwt_auth_refresh_ttl',
			self::SETTINGS_TYPE_STRING );
	}
	...
	private function validateAuthenticationConfigFromPost() {
		if ( (int) $this->post['allow_authentication'] === 1 && ( empty( $this->post['jwt_payload'] ) ) ) {
			throw new \Exception(
                ...
                SettingsErrors::generateCode(SettingsErrors::PREFIX_AUTHENTICATION, SettingsErrors::ERR_AUTHENTICATION_EMPTY_PAYLOAD)
			);
		}
		...
		if ( empty( (int) $this->post['jwt_auth_ttl'] ) || (int) $this->post['jwt_auth_ttl'] < 0 ) {
			throw new \Exception(
                ...
                SettingsErrors::generateCode(SettingsErrors::PREFIX_AUTHENTICATION, SettingsErrors::ERR_AUTHENTICATION_TTL)
			);
		}
		...
		if ( empty( (int) $this->post['jwt_auth_refresh_ttl'] ) || (int) $this->post['jwt_auth_refresh_ttl'] < 0 ) {
			throw new \Exception(
                ...
                SettingsErrors::generateCode(SettingsErrors::PREFIX_AUTHENTICATION, SettingsErrors::ERR_AUTHENTICATION_REFRESH_TTL_ZERO)
			);
		}
	}
	...
	private function initHooksConfigFromPost() {
		$this->assignSettingsPropertyFromPost( null,'enabled_hooks', null,'enabled_hooks', self::SETTINGS_TYPE_ARRAY );
	}

	private function initGeneralSettingsFromPost() {
		$this->assignSettingsPropertyFromPost( null,'route_namespace', null,'route_namespace', self::SETTINGS_TYPE_STRING );
		$this->assignSettingsPropertyFromPost( null,'jwt_algorithm', null,'jwt_algorithm', self::SETTINGS_TYPE_STRING );
		$this->assignSettingsPropertyFromPost( null,'decryption_source', null,'decryption_source', self::SETTINGS_TYPE_STRING );
		$this->assignSettingsPropertyFromPost( null,'decryption_key', null,'decryption_key', self::SETTINGS_TYPE_STRING );
		$this->assignSettingsPropertyFromPost( null,'decryption_key_base64', null,'decryption_key_base64', self::SETTINGS_TYPE_BOL,false );
		$this->assignSettingsPropertyFromPost( null,'decryption_key_public', null,'decryption_key_public', self::SETTINGS_TYPE_STRING,null,true);
		$this->assignSettingsPropertyFromPost( null,'decryption_key_private', null,'decryption_key_private', self::SETTINGS_TYPE_STRING,null,true);
		$this->assignSettingsPropertyFromPost( null,'request_jwt_url', null,'request_jwt_url', self::SETTINGS_TYPE_INT );
	...
	private function validateGeneralSettingsFromPost() {
	    ...
	    if(!empty($this->post['jwt_algorithm'])){
            if(isset($this->post['decryption_source']) && $this->post['decryption_source'] === self::DECRYPTION_SOURCE_CODE){
                if(strpos($this->post['jwt_algorithm'],'RS')!==false
                    && (!defined(JwtKeyWpConfig::SIMPLE_JWT_PUBLIC_KEY) || !defined(JwtKeyWpConfig::SIMPLE_JWT_PRIVATE_KEY))){
                        throw new \Exception(
                            __('Public or private key is not defined in code.', 'simple-jwt-login'),
                            SettingsErrors::generateCode(SettingsErrors::PREFIX_GENERAL, SettingsErrors::ERR_GENERAL_PRIVATE_KEY_MISSING_FROM_CODE_RS)
                        );
                }else if(!defined(JwtKeyWpConfig::SIMPLE_JWT_PRIVATE_KEY)){
                    throw new \Exception(
                        __('Private key is not defined in code.','simple-jwt-login'),
                        SettingsErrors::generateCode(SettingsErrors::PREFIX_GENERAL, SettingsErrors::ERR_GENERAL_PRIVATE_KEY_NOT_PRESENT_IN_CODE_HS)
                    );
                }
            }else {
                if (strpos($this->post['jwt_algorithm'], 'RS') !== false) {
                    if (
                        !isset($this->post['decryption_key_public']) || empty(trim($this->post['decryption_key_public']))
                        || !isset($this->post['decryption_key_private']) || empty(trim($this->post['decryption_key_private']))
                    ) {
                        throw  new \Exception(
                            __('JWT Decryption public and private key are required.', 'simple-jwt-login'),
                        SettingsErrors::generateCode(SettingsErrors::PREFIX_GENERAL, SettingsErrors::ERR_GENERAL_MISSING_PRIVATE_AND_PUBLIC_KEY)
                        );
                    }
                } else {
                    if (!isset($this->post['decryption_key']) || empty(trim($this->post['decryption_key']))) {
                        throw  new \Exception(
                            ...
                            SettingsErrors::generateCode(SettingsErrors::PREFIX_GENERAL, SettingsErrors::ERR_GENERAL_DECRYPTION_KEY_REQUIRED)
                        );
                    }
                }
            }
		...
		if (
			empty( $this->post['request_jwt_url'] )
	...
	private function initAuthCodesFromPost() {
		...
		if ( isset( $this->post['auth_codes'] ) && isset($this->post['auth_codes']['code'])) {
			...
			foreach ( $codes as $key => $code ) {
				if(trim($code) === '' || !isset($this->post['auth_codes']['role'][$key]) || !isset($this->post['auth_codes']['expiration_date'][$key])) {
					continue;
				}
				$authCodes[] = [
					...
					'expiration_date' => $this->wordPressData->sanitize_text_field($this->post['auth_codes']['expiration_date'][$key])
				];
			}
		}
	...
	private function validateAuthCodesFromPost() {
		...
		$x =1;
		if (
			! empty( $this->settings['require_login_auth'] ) && ! empty( $this->settings['allow_autologin'] )
			...
			if ( empty( $this->settings['auth_codes'] ) ) {
				throw new \Exception(
                    ...
                    SettingsErrors::generateCode(SettingsErrors::PREFIX_AUTH_CODES, SettingsErrors::ERR_EMPTY_AUTH_CODES)
				);
			}
		}
	}
	...
	private function initLoginConfigFromPost() {
		$this->assignSettingsPropertyFromPost( null,'jwt_login_by',null, 'jwt_login_by', self::SETTINGS_TYPE_INT );
		$this->assignSettingsPropertyFromPost( null,'jwt_login_by_parameter', null,'jwt_login_by_parameter',
			self::SETTINGS_TYPE_STRING );
		$this->assignSettingsPropertyFromPost( null,'allow_autologin',null, 'allow_autologin', self::SETTINGS_TYPE_BOL );
	...
	private function validateLoginConfigFromPost() {
        if (
            isset( $this->post['allow_autologin'] ) && (int) $this->post['allow_autologin'] === 1
            ...
            throw  new \Exception(
                ...
                SettingsErrors::generateCode(SettingsErrors::PREFIX_LOGIN, SettingsErrors::ERR_LOGIN_MISSING_JWT_PARAMETER_KEY)
            );
        }
	...
	private function initRegisterConfigFromPost() {
		$this->assignSettingsPropertyFromPost( null,'allow_register', null,'allow_register', self::SETTINGS_TYPE_BOL );
		$this->assignSettingsPropertyFromPost( null, 'new_user_profile',null, 'new_user_profile', self::SETTINGS_TYPE_STRING );
		$this->assignSettingsPropertyFromPost( null, 'login_ip',null, 'login_ip', self::SETTINGS_TYPE_STRING );
		$this->assignSettingsPropertyFromPost( null, 'register_ip',null, 'register_ip', self::SETTINGS_TYPE_STRING );
		$this->assignSettingsPropertyFromPost( null, 'register_domain',null, 'register_domain', self::SETTINGS_TYPE_STRING );
		$this->assignSettingsPropertyFromPost( null, 'require_register_auth', null,'require_register_auth', self::SETTINGS_TYPE_BOL );
		$this->assignSettingsPropertyFromPost( null, 'allowed   ', null,'require_register_auth', self::SETTINGS_TYPE_BOL );
	}
    ...
    private function validateRegisterUser(){
        if(empty($this->post['new_user_profile'])){
            throw new \Exception(
                ...
                SettingsErrors::generateCode(SettingsErrors::PREFIX_REGISTER, SettingsErrors::ERR__REGISTER_MISSING_NEW_USER_PROFILE)
            );
        }
    }
	...
	private function initDeleteUserConfigFromPost() {
		...
		$this->assignSettingsPropertyFromPost( null,'delete_ip',null, 'delete_ip', self::SETTINGS_TYPE_STRING );
		$this->assignSettingsPropertyFromPost( null,'allowed_user_meta',null, 'allowed_user_meta', self::SETTINGS_TYPE_STRING );
		$this->assignSettingsPropertyFromPost( null,'delete_user_by', null,'delete_user_by', self::SETTINGS_TYPE_INT );
		$this->assignSettingsPropertyFromPost( null,'jwt_delete_by_parameter', null,'jwt_delete_by_parameter',
			self::SETTINGS_TYPE_STRING );
		$this->assignSettingsPropertyFromPost( null,'random_password', null,'random_password', self::SETTINGS_TYPE_BOL, false );
		$this->assignSettingsPropertyFromPost( null,'register_force_login', null,'register_force_login', self::SETTINGS_TYPE_BOL,
			false );
	}
	...
	private function validateDeleteUserConfigFromPost() {
		if (
			...
			throw new \Exception(
                ...
                SettingsErrors::generateCode(SettingsErrors::PREFIX_DELETE, SettingsErrors::ERR_DELETE_MISSING_JWT_PARAM)
            );
		}
	}
	...
	private function initCorsFromPost() {
		$this->assignSettingsPropertyFromPost('cors','enabled','cors','enabled',self::SETTINGS_TYPE_INT);

		$this->assignSettingsPropertyFromPost('cors','allow_origin_enabled','cors','allow_origin_enabled',self::SETTINGS_TYPE_BOL, false);
		$this->assignSettingsPropertyFromPost('cors','allow_origin','cors','allow_origin',self::SETTINGS_TYPE_STRING);

		$this->assignSettingsPropertyFromPost('cors','allow_methods_enabled','cors','allow_methods_enabled',self::SETTINGS_TYPE_BOL, false);
		$this->assignSettingsPropertyFromPost('cors','allow_methods','cors','allow_methods',self::SETTINGS_TYPE_STRING);

		$this->assignSettingsPropertyFromPost('cors','allow_headers_enabled','cors','allow_headers_enabled',self::SETTINGS_TYPE_BOL, false);
		$this->assignSettingsPropertyFromPost('cors','allow_headers','cors','allow_headers',self::SETTINGS_TYPE_STRING);
	}

	private function validateCorsFromPost() {
		if(
			!empty($this->post['cors']['enabled'])
			&& (
